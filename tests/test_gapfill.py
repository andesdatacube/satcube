...
def test_load_ref_caches(tmp_path):
    ...
    _write_tif(p, _scene())
    cache = {}
    ref1, valid1 = _load_ref(p, cache)
    ref2, valid2 = _load_ref(p, cache)
    assert ref1 is ref2
    assert p in cache
    assert valid1.shape == (32, 32)
...
def test_fill_one_no_gaps_returns_zero(tmp_path):
    ...
    stem, gaps = _fill_one(
        img_path=p, ref_paths=[p], dates=dates, this_date=dates[0],
...
def test_fill_one_fills_hole(tmp_path):
    ...
    _write_tif(ptgt, tgt)
    pref = tmp_path / "r.tif"
    _write_tif(pref, _scene(seed=2))
    dates = np.array(["2022-01-01", "2022-01-06"], dtype="datetime64[ns]")
    stem, gaps = _fill_one(
        img_path=ptgt, ref_paths=[ptgt, pref], dates=dates, this_date=dates[0],
...
def test_gapfill_fn_end_to_end(tmp_path):
    in_dir = tmp_path / "masked"
    out_dir = tmp_path / "gapfilled"
    in_dir.mkdir()
    ids = ["s0", "s1", "s2"]
    dates = ["2022-01-01", "2022-01-06", "2022-01-11"]
    ...
    assert "remaining_gaps_pct" in out.columns
    assert len(out) == 3
    for i in ids:
        assert (out_dir / f"{i}.tif").exists()
    assert not (out_dir / "_round0").exists()
    assert not (out_dir / "_round1").exists()